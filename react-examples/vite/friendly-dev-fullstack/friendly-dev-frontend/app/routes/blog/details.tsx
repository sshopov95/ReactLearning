...
import type { PostMeta, BlogPostData, StrapiResponse, StrapiPost } from '~/types';
import { Link } from 'react-router';

export async function loader({request, params}:Route.LoaderArgs) {
    const {slug} = params;

    const res = await fetch(`${import.meta.env.VITE_API_URL}/posts?[slug][$eq]=${slug}&populate=image`)
